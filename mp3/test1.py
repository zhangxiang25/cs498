...
class Experiment:
    ...
    def get_vase_position(self):
        """获取花瓶在世界坐标系中的位置"""
        vase_body_idx = self.scene["vase"].find_bodies("vase")[0][0]  # 获取花瓶主体ID
        vase_pose = self.scene["vase"].data.body_state_w[0, vase_body_idx, :3].detach().cpu().numpy()
        print(f"Vase position detected: {vase_pose}")
        return vase_pose


    def compute_camera_poses(self, vase_pos, radius=0.3, height=0.4, num_positions=5):
        """计算围绕花瓶的5个相机位姿"""
        camera_poses = []
